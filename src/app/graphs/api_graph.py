...
from fastapi.concurrency import run_in_threadpool # For blocking I/O like RabbitMQ sync client
...
async def prepare_and_validate_input_node(state: ApiGraphState) -> Dict[str, Any]:
    logger.info(f"API Graph Node: prepare_and_validate_input_node for user {state['user_id']}")
    ...
    if state.get("input_files"):
        ...
        for file_obj in state["input_files"]:
            ...
            if not file_obj.filename or not file_obj.content:
                return {"db_error": "Invalid file data: filename and content are required."}
            files_to_save.append({"filename": file_obj.filename, "content": file_obj.content})
        ...
        files_to_save.append({"filename": f"snippet.{primary_language}", "content": state["input_code"]})
    else:
        logger.error("No valid input (code+language or files) provided to API graph.")
        return {"db_error": "Invalid input: Must provide code with language or a list of files."}
...
async def save_submission_to_db_node(state: ApiGraphState) -> Dict[str, Any]:
    logger.info(f"API Graph Node: save_submission_to_db_node for user {state['user_id']}")
    ...
    async with AsyncSessionLocal() as session:
        async with session.begin():
            try:
                ...
                logger.info(f"Successfully saved CodeSubmission {new_submission.id} and associated files to DB.")
                return {"submission_id": new_submission.id, "db_error": None}
            except Exception as e:
                await session.rollback()
                logger.error(f"Error saving submission to DB: {e}", exc_info=True)
                return {"db_error": f"Database save failed: {str(e)}", "submission_id": None}


async def publish_to_mq_node(state: ApiGraphState) -> Dict[str, Any]:
    logger.info(f"API Graph Node: publish_to_mq_node for submission_id {state.get('submission_id')}")
    ...
    try:
        ...
        logger.info(f"Successfully published submission_id {submission_id} to queue '{CODE_QUEUE}'.")
        return {
            "publish_error": None,
        ...
        return {"publish_error": "JSON serialization error for MQ.", "final_message": "Internal error with MQ."}
    except Exception as e: # Catch errors from publish_to_rabbitmq
        logger.error(f"Error publishing submission_id {submission_id} to MQ: {e}", exc_info=True)
        return {"publish_error": str(e), "final_message": f"Failed to queue submission: {e}"}
...
def route_after_db_save(state: ApiGraphState) -> Literal["publish_to_mq", "__end__"]:
    if state.get("db_error") or state.get("submission_id") is None:
        logger.error(f"Routing to END after DB save due to error or no submission_id. DB Error: {state.get('db_error')}")
