...
from typing import Any, Callable, Optional # Added Optional
...
logging.basicConfig(
    level=logging.DEBUG, # Lowered to DEBUG for more verbose output from this module
    format="%(asctime)s - %(levelname)-7s - [%(threadName)s] %(name)s (%(module)s.%(funcName)s:%(lineno)d) - %(message)s"
)
logger = logging.getLogger(__name__) # Gets logger for current module __main__ if run directly
...
async def run_graph_task_wrapper(initial_state: WorkerGraphState, delivery_tag: int):
    ...
    logger.info(f"ASYNC WRAPPER: Starting actual worker_workflow for submission_id: {submission_id}, delivery_tag: {delivery_tag}")

    success = False
    final_graph_state: Optional[WorkerGraphState] = None # To store the result from the graph

    try:
        # === CALL THE ACTUAL WORKER GRAPH ===
        logger.info(f"ASYNC WRAPPER: Invoking worker_workflow for SID: {submission_id}...")
        final_graph_state = await worker_workflow.ainvoke(initial_state)
        logger.info(f"ASYNC WRAPPER: worker_workflow completed for SID: {submission_id}.")

        if final_graph_state:
            db_status = final_graph_state.get("db_save_status")
            graph_error = final_graph_state.get("error_message")
            logger.info(f"ASYNC WRAPPER: Graph result for SID {submission_id} - DB_Status: {db_status}, Error: {graph_error}")
            if db_status == "Success" and not graph_error:
                success = True
            else:
                logger.error(f"ASYNC WRAPPER: Graph processing for SID {submission_id} reported issues. DB Status: {db_status}, Error: {graph_error}")
                success = False # Ensure success is false if db_status is not "Success" or error exists
        else:
            logger.error(f"ASYNC WRAPPER: worker_workflow returned None for SID {submission_id}.")
            success = False
        ...
        logger.error(f"ASYNC WRAPPER: Exception during worker_workflow invocation for SID {submission_id}: {e}", exc_info=True)
        success = False
        # Create a minimal final_graph_state for error reporting
        if final_graph_state is None: # If error happened before final_graph_state was set
    ...
    if _pika_channel and _pika_channel.is_open and _pika_connection and _pika_connection.is_open:
        def pika_finalize_message():
            try:
                if success:
                    logger.info(f"PIKA FINALIZE (via threadsafe_cb): ACK delivery_tag {delivery_tag} for SID {submission_id}")
                    if _pika_channel and _pika_channel.is_open: _pika_channel.basic_ack(delivery_tag=delivery_tag)
                else:
                    error_info = final_graph_state.get("error_message", "Processing failed") if final_graph_state else "Processing failed (no final state)"
                    logger.error(f"PIKA FINALIZE (via threadsafe_cb): NACK delivery_tag {delivery_tag} for SID {submission_id}. Reason: {error_info}")
                    if _pika_channel and _pika_channel.is_open: _pika_channel.basic_nack(delivery_tag=delivery_tag, requeue=False)
            except Exception as e_pika_finalize:
                logger.error(f"PIKA FINALIZE (via threadsafe_cb): Exception for SID {submission_id}, delivery_tag {delivery_tag}: {e_pika_finalize}", exc_info=True)

        _pika_connection.add_callback_threadsafe(pika_finalize_message)
    else:
        logger.error(f"ASYNC WRAPPER: Pika channel/connection not open. Cannot ACK/NACK SID {submission_id}, delivery_tag {delivery_tag}.")
...
def schedule_task_on_async_loop(target_coroutine_func: Callable, *args, **kwargs):
    ...
    if _async_loop and _async_loop.is_running():
        ...
        def _scheduler():
            ...
            logger.debug(f"ASYNC SCHEDULER (via _scheduler): Task created for {target_coroutine_func.__name__}")

        _async_loop.call_soon_threadsafe(_scheduler)
        logger.info(f"ASYNC SCHEDULER: Task for {target_coroutine_func.__name__} scheduled via call_soon_threadsafe.")
    else:
        logger.error(f"ASYNC SCHEDULER: Asyncio loop not available/running. Task for {target_coroutine_func.__name__} not scheduled.")


def pika_message_callback(ch: Channel, method: Basic.Deliver, properties: pika.spec.BasicProperties, body: bytes):
    """Pika callback for received messages."""
    submission_id_str = "N/A (before parse)"
    logger.info(f"PIKA CB: Received message. Delivery Tag: {method.delivery_tag}. Body (first 100): {body[:100]}")
    ...
    try:
        ...
        if submission_id is None:
            logger.error(f"PIKA CB: Invalid message - no submission_id. Body: {body.decode('utf-8')}")
            if ch.is_open: ch.basic_nack(delivery_tag=method.delivery_tag, requeue=False)
            return

        logger.info(f"PIKA CB: Processing submission_id: {submission_id}. Scheduling async task wrapper.")
        initial_state = WorkerGraphState(
            submission_id=submission_id, final_report=None, db_save_status=None, 
            error_message=None, files_data=None, primary_language=None
        )
        
        # Schedule run_graph_task_wrapper to be executed on the asyncio loop
        schedule_task_on_async_loop(run_graph_task_wrapper, initial_state, method.delivery_tag)

    except json.JSONDecodeError:
        logger.error(f"PIKA CB: Failed to decode JSON: {body.decode('utf-8')}", exc_info=True)
        if ch.is_open: ch.basic_nack(delivery_tag=method.delivery_tag, requeue=False)
    except Exception as e:
        logger.error(f"PIKA CB: Unhandled error for SID '{submission_id_str}': {e}", exc_info=True)
        if ch.is_open: ch.basic_nack(delivery_tag=method.delivery_tag, requeue=False)
...
def asyncio_thread_worker_target():
    ...
    try:
        ...
        logger.info("ASYNCIO THREAD: KeyboardInterrupt received in asyncio thread (should be handled by main Pika thread).")
    finally:
        logger.info("ASYNCIO THREAD: loop.run_forever() exited.")
        # Gracefully shutdown any remaining tasks
        if _async_loop.is_running(): # Should be false if stop() was called and processed
             logger.warning("ASYNCIO THREAD: Loop still shows as running after run_forever exited; attempting to stop again.")
             _async_loop.call_soon_threadsafe(_async_loop.stop) # Should have been called already
        ...
        try:
            ...
            if tasks_to_cancel:
                logger.info(f"ASYNCIO THREAD: Cancelling {len(tasks_to_cancel)} outstanding tasks.")
                for task in tasks_to_cancel:
                    task.cancel()
                # Wait for tasks to cancel
                _async_loop.run_until_complete(asyncio.gather(*tasks_to_cancel, return_exceptions=True))
                logger.info("ASYNCIO THREAD: Outstanding tasks cancelled.")
            else:
                logger.info("ASYNCIO THREAD: No outstanding tasks to cancel.")
        except Exception as e_shutdown:
            logger.error(f"ASYNCIO THREAD: Error during task cleanup on shutdown: {e_shutdown}", exc_info=True)
...
def start_worker_consumer():
    ...
    if not (_event_loop_thread and _event_loop_thread.is_alive()):
        _event_loop_thread = threading.Thread(target=asyncio_thread_worker_target, name="AsyncioEventLoopThread", daemon=True)
        ...
        if not (_async_loop and _async_loop.is_running()):
            logger.error("WORKER: Asyncio loop did not start running in dedicated thread. Aborting worker.")
            if _event_loop_thread.is_alive(): # Signal it to stop if it's stuck before run_forever
                 if _async_loop: _async_loop.call_soon_threadsafe(_async_loop.stop)
    ...
    while not _stop_event.is_set():
        try:
            logger.info(f"WORKER: Attempting RabbitMQ connection to {RABBITMQ_HOST}:{RABBITMQ_PORT}...")
            ...
            _pika_channel.basic_consume(queue=CODE_QUEUE, on_message_callback=pika_message_callback, auto_ack=False)

            logger.info(f"WORKER: Waiting for messages in '{CODE_QUEUE}'. CTRL+C to exit.")
            ...
            if _pika_channel and _pika_channel.is_open:
                try: 
                    ...
                    _pika_channel.close()
                except Exception as e_ch_close: logger.error(f"WORKER: Error closing Pika channel: {e_ch_close}")
            if _pika_connection and _pika_connection.is_open:
                try: _pika_connection.close()
                except Exception as e_conn_close: logger.error(f"WORKER: Error closing Pika connection: {e_conn_close}")
            logger.info("WORKER: Pika connection/channel (attempted) closed for this iteration.")
        ...
        logger.info(f"WORKER: Waiting {retry_delay}s before retrying RabbitMQ connection.")
    ...
    if _async_loop and _async_loop.is_running(): # Ensure it's running before trying to stop
