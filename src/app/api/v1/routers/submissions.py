...
from app.shared.lib.archive_utils import extract_archive_to_files
...
async def submit_code(
    ...
    common_args = {
        ...
        "excluded_files": [f.strip() for f in excluded_files.split(",")]
        if excluded_files
        else [],
        "main_llm_id": main_llm_config_id,
        "specialized_llm_id": specialized_llm_config_id,
    }
    if files:
...
async def trigger_remediation(
    ...
    await service.queue_remediation(
        submission_id, remediation_request, user, correlation_id_var.get()
    )
...
@router.get(
    "/result/{submission_id}", response_model=api_models.AnalysisResultDetailResponse
)
async def get_submission_results(
    submission_id: uuid.UUID,
    service: SubmissionService = Depends(get_submission_service),
):
    # This logic is now complex enough to warrant being in the service layer
    return await service.get_results(submission_id)


# PDF download endpoint can stay here as it's primarily an HTTP response construction task
# Or be moved to a "ReportingService" in the future.
