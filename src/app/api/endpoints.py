...
import logging
import uuid # For User.id type hint
...
async def analyze_code_submission(
    ...
    logger.info(f"Received /analyze request from user ID: {user.id}. Language hint: {payload.language}")
    ...
    try:
        logger.debug(f"Invoking API graph workflow with initial state for user {user.id}.")
        final_state = await api_workflow.ainvoke(initial_state)
        logger.info(f"API graph workflow completed for user {user.id}. Final state message: {final_state.get('final_message')}")

        if final_state.get("db_error"):
            logger.error(f"DB error from API graph: {final_state['db_error']}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=final_state.get("final_message") or "Database operation failed during submission."
            )

        if final_state.get("publish_error"):
            logger.error(f"MQ publish error from API graph: {final_state['publish_error']}")
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=final_state.get("final_message") or "Failed to queue submission for analysis."
            )

        submission_id = final_state.get("submission_id")
        if submission_id is not None:
            return {
                "message": final_state.get("final_message", "Submission accepted and queued."),
                "submission_id": submission_id
            }
        else:
            # This case should ideally be covered by db_error or publish_error
            logger.error(f"API graph completed without submission_id and no explicit error for user {user.id}.")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        logger.error(f"Unexpected error during /analyze endpoint for user {user.id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_analysis_results(
    ...
    if submission_owner_check.scalar_one_or_none() is None:
        logger.warning(f"User {user.id} attempted to access unauthorized or non-existent submission {submission_id}.")
        raise HTTPException(
    ...
    if analysis_result is None:
        ...
        logger.info(f"Analysis result not yet available for submission ID: {submission_id} (owned by user {user.id}).")
    ...
    logger.info(f"Returning analysis result for submission ID: {submission_id} to user {user.id}.")
