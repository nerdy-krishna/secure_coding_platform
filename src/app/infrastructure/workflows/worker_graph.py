...
from app.infrastructure.agents.context_analysis_agent import (
...
async def retrieve_submission_data(state: WorkerState) -> Dict[str, Any]:
    ...
    return_value: Dict[str, Any] = {
        "error_message": f"Failed to retrieve data for submission {submission_id}."
    }
    ...
    try:
        ...
        async for db in get_db():
            ...
            if not submission:
                return_value = {
                    "error_message": f"Submission with ID {submission_id} not found."
                }
                break  # Exit loop, will return default or this specific error

            submitted_files = await crud.get_submitted_files_for_submission(
                db, submission_id
            )
            if not submitted_files:
                return_value = {
                    "error_message": f"No files found for submission ID {submission_id}."
                }
                break  # Exit loop
        ...
        logger.error(
            f"[WorkerGraph] Error retrieving data for submission {submission_id}: {e}",
            exc_info=True,
        )
...
async def run_impact_reporting(state: WorkerState) -> Dict[str, Any]:
    ...
    logger.info(
        f"[WorkerGraph] Preparing inputs and running ImpactReportingAgent for {state['submission_id']}."
    )
    ...
    reporting_input_state: ImpactReportingAgentState = {
        ...
        "findings": state.get(
            "findings", []
        ),  # Access top-level findings populated by CoordinatorAgent
    ...
    if report_output_state.get("error"):
        error_msg = (
            f"ImpactReportingAgent sub-graph failed: {report_output_state['error']}"
        )
...
async def save_final_report_node(state: WorkerState) -> Dict[str, Any]:
    ...
    if not impact_report and not sarif_report:
        logger.warning(
            "[WorkerGraph] No reports were found in state. The submission will be marked 'Completed' without reports."
        )
...
async def handle_error_node(state: WorkerState) -> Dict[str, Any]:
    ...
    submission_id = state["submission_id"]
    logger.error(
        f"[WorkerGraph] Workflow for submission {submission_id} failed: {error}"
    )
    async for db in get_db():
...
async def check_approval_status_node(state: WorkerState) -> Dict[str, Any]:  # type: ignore
    ...
    submission_id = state["submission_id"]
    logger.info(
        f"[WorkerGraph] Checking approval status for submission {submission_id} after coordinator."
    )
    ...
    output_state = {
        **state.get("analysis_results", {}),
        "current_submission_status": "UNKNOWN_DB_ERROR",
    }
    ...
    async for db in get_db():
        try:
            ...
            if not submission:
                logger.error(
                    f"[WorkerGraph] Submission {submission_id} not found during status check."
                )
                return {
                    **output_state,
            ...
            logger.info(
                f"[WorkerGraph] Submission {submission_id} current status from DB: {current_status}."
            )
            return {
                **output_state,
            ...
            logger.error(
                f"[WorkerGraph] DB Error checking status for {submission_id}: {e}",
                exc_info=True,
            )
            ...
            return {
                **output_state,
                ...
                "current_submission_status": "ERROR_DB_READ",
            }
...
def route_after_coordinator_check(state: WorkerState) -> str:
    ...
    if current_status == STATUS_PENDING_APPROVAL:
        logger.info(
            f"[WorkerGraph] Submission {submission_id} is {STATUS_PENDING_APPROVAL}. Pausing worker graph."
        )
        return END

    logger.info(
        f"[WorkerGraph] Submission {submission_id} status is '{current_status}'. Proceeding to reporting."
    )
...
async def get_workflow() -> Pregel:
    ...
    if (
        _checkpointer_conn is None or _checkpointer_conn.closed
    ):  # Changed is_closed() to closed
    ...
    workflow.add_node(
        "context_analysis",
        context_analysis_graph.with_config(run_name="ContextAnalysisAgent"),
    )
    workflow.add_node(
        "coordinator", coordinator_graph.with_config(run_name="CoordinatorAgent")
    )
    ...
    workflow.add_conditional_edges(
        "retrieve_submission_data",
        should_continue,
        {"continue": "context_analysis", "handle_error": "handle_error"},
    )
    workflow.add_conditional_edges(
        "context_analysis",
        should_continue,
    ...
    workflow.add_conditional_edges(
        "coordinator",
        should_continue,
        {"continue": "check_approval_status", "handle_error": "handle_error"},
    )
...
async def close_workflow_resources():
    ...
    if (
        _checkpointer_conn and not _checkpointer_conn.closed
    ):  # Changed is_closed() to closed
