...
from app.shared.analysis_tools.context_bundler import (
...
from app.infrastructure.agents.access_control_agent import (
    build_specialized_agent_graph as build_access_control_agent,
)
from app.infrastructure.agents.api_security_agent import (
    build_specialized_agent_graph as build_api_security_agent,
)
from app.infrastructure.agents.architecture_agent import (
    build_specialized_agent_graph as build_architecture_agent,
)
from app.infrastructure.agents.authentication_agent import (
    build_specialized_agent_graph as build_authentication_agent,
)
from app.infrastructure.agents.business_logic_agent import (
    build_specialized_agent_graph as build_business_logic_agent,
)
from app.infrastructure.agents.code_integrity_agent import (
    build_specialized_agent_graph as build_code_integrity_agent,
)
from app.infrastructure.agents.communication_agent import (
    build_specialized_agent_graph as build_communication_agent,
)
from app.infrastructure.agents.configuration_agent import (
    build_specialized_agent_graph as build_configuration_agent,
)
from app.infrastructure.agents.cryptography_agent import (
    build_specialized_agent_graph as build_cryptography_agent,
)
from app.infrastructure.agents.data_protection_agent import (
    build_specialized_agent_graph as build_data_protection_agent,
)
from app.infrastructure.agents.error_handling_agent import (
    build_specialized_agent_graph as build_error_handling_agent,
)
from app.infrastructure.agents.file_handling_agent import (
    build_specialized_agent_graph as build_file_handling_agent,
)
from app.infrastructure.agents.session_management_agent import (
    build_specialized_agent_graph as build_session_management_agent,
)
from app.infrastructure.agents.validation_agent import (
...
STATUS_COST_APPROVED = (
    "Approved - Queued"  # This status is set by the API upon user approval
)
...
def determine_relevant_agents_node(state: CoordinatorState) -> Dict[str, Any]:
    ...
    if not repository_map:
        return {
            "error": "Repository map is missing, cannot determine agent assignments."
        }
...
def create_context_bundles_node(state: CoordinatorState) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"[{AGENT_NAME}] Successfully created {len(bundles)} context bundles."
        )
        return {"context_bundles": bundles}
    except Exception as e:
        logger.error(
            f"[{AGENT_NAME}] Failed to create context bundles: {e}", exc_info=True
        )
...
async def estimate_cost_node(state: CoordinatorState) -> Dict[str, Any]:
    ...
    logger.info(
        f"[{AGENT_NAME}] Evaluating cost and approval status for submission {submission_id}."
    )
    ...
    async with async_session_factory() as db:
        ...
        if not submission:
            logger.error(
                f"[{AGENT_NAME}] Submission {submission_id} not found during cost evaluation."
            )
            return {
                "error": f"Submission {submission_id} not found.",
                "current_submission_status": "ERROR_NO_SUBMISSION",
            }
        ...
        if current_status == STATUS_COST_APPROVED:
            logger.info(
                f"[{AGENT_NAME}] Cost for submission {submission_id} is already approved ({STATUS_COST_APPROVED}). Proceeding."
            )
            return {
                "cost_approval_met": True,
                "current_submission_status": current_status,
                "estimated_cost": estimated_cost_from_db,
            }
        ...
        if current_status == STATUS_PENDING_APPROVAL:
            logger.info(
                f"[{AGENT_NAME}] Submission {submission_id} is still {STATUS_PENDING_APPROVAL}. Waiting for external approval."
            )
            return {
                "cost_approval_met": False,
                "current_submission_status": current_status,
                "estimated_cost": estimated_cost_from_db,
            }
        ...
        logger.info(
            f"[{AGENT_NAME}] Performing new cost estimation for submission {submission_id}."
        )
        ...
        if not bundles or not llm_config_id:
            logger.error(
                f"[{AGENT_NAME}] Bundles or LLM config ID missing for submission {submission_id}."
            )
            return {
                "error": "Bundles or LLM config ID missing, cannot estimate cost.",
                "current_submission_status": current_status,
            }
        ...
        try:
            ...
            if not llm_config:
                logger.error(
                    f"[{AGENT_NAME}] LLM Config {llm_config_id} not found for submission {submission_id}."
                )
                return {
                    "error": f"LLM Configuration with ID {llm_config_id} not found.",
                    "current_submission_status": current_status,
                }
            ...
            logger.info(
                f"[{AGENT_NAME}] Total estimated input tokens for {submission_id}: {total_input_tokens}"
            )

            cost_details = cost_estimation.estimate_cost_for_prompt(
                config=llm_config, input_tokens=total_input_tokens
            )

            await crud.update_submission_cost_and_status(
                db, submission_id, STATUS_PENDING_APPROVAL, cost_details
            )
            logger.info(
                f"[{AGENT_NAME}] Submission {submission_id} status set to {STATUS_PENDING_APPROVAL}. Estimated cost: {cost_details['total_estimated_cost']:.6f} USD."
            )
            return {
                "estimated_cost": cost_details,
                "cost_approval_met": False,
                "current_submission_status": STATUS_PENDING_APPROVAL,
            }
        except Exception as e:
            logger.error(
                f"[{AGENT_NAME}] Failed to estimate cost for {submission_id}: {e}",
                exc_info=True,
            )
            return {
...
def route_after_cost_estimation(state: CoordinatorState) -> str:
    ...
    if state.get("error"):
        logger.error(
            f"[{AGENT_NAME}] Error in coordinator state after cost estimation: {state['error']}"
        )
        return "end_with_error"

    if state.get("cost_approval_met"):  # True if status was COST_APPROVED
        logger.info(
            f"[{AGENT_NAME}] Cost approval met for submission {state['submission_id']}. Proceeding to specialized agents."
        )
    ...
    logger.info(
        f"[{AGENT_NAME}] Cost approval not met (status: {state.get('current_submission_status')}) for {state['submission_id']}. Coordinator ending."
    )
    return END
...
async def run_specialized_agents_node(state: CoordinatorState) -> Dict[str, Any]:
    ...
    logger.info(
        f"[{AGENT_NAME}] Beginning specialized agent runs in '{workflow_mode}' mode with a concurrency limit of {CONCURRENT_LLM_LIMIT}."
    )

    if not context_bundles or not files:
        return {
            "error": "Context bundles or original files not found, cannot run specialized agents."
        }
    ...
    if workflow_mode == "audit":
        ...
        bundle_map: Dict[str, ContextBundle] = {
            b.target_file_path: b for b in context_bundles
        }
        tasks = []
        ...
        for result in agent_results:
            if isinstance(result, dict):
                ...
                logger.error(
                    f"[{AGENT_NAME}] An agent task failed during audit: {result}",
                    exc_info=result,
                )
    ...
    # --- Mode: 'remediate' (Sequential Execution) ---
        ...
        for agent_name, file_paths in relevant_agents.items():
            ...
            for file_path in file_paths:
                ...
                logger.info(
                    f"[{AGENT_NAME}] Sequentially remediating '{file_path}' with '{agent_name}'."
                )
                    ...
                    for fix_result in fixes_from_run:
                        ...
                        if target_file in live_codebase:
                            ...
                            if original_snippet in file_content:
                                ...
                                live_codebase[target_file] = file_content.replace(
                                    original_snippet, new_code, 1
                                )
                                logger.info(f"Applied fix in '{target_file}'.")
                            else:
                                logger.warning(
                                    f"Could not find original snippet in '{target_file}' to apply fix. The code may have been changed by a previous fix."
                                )
                elif isinstance(agent_result, Exception):
                    logger.error(
                        f"[{AGENT_NAME}] An agent task failed during remediation: {agent_result}",
                        exc_info=agent_result,
                    )

        logger.info(
            f"[{AGENT_NAME}] Collated {len(all_findings)} findings and {len(all_fixes)} fixes from remediation."
        )
        return {
            "findings": all_findings,
            "fixes": all_fixes,
            "live_codebase": live_codebase,
        }
...
async def finalize_analysis_node(state: CoordinatorState) -> Dict[str, Any]:
    ...
    logger.info(
        f"[{AGENT_NAME}] Finalizing analysis for submission {submission_id} in '{workflow_mode}' mode."
    )
    ...
    async with async_session_factory() as db:
        try:
            if findings_to_save:
                ...
                if fixes_to_save:
                    finding_map = {
                        (f.file_path, f.line_number, f.cwe): f.id
                    ...
                    logger.info(
                        f"[{AGENT_NAME}] Saved {len(fixes_to_save)} fix suggestions."
                    )
            ...
            if workflow_mode == "remediate" and live_codebase:
                await crud.update_remediated_code_and_status(
                    db, submission_id, "Remediation-Completed", live_codebase
                )
                logger.info(
                    f"[{AGENT_NAME}] Saved fixed code map and updated status to 'Remediation-Completed'."
                )
            else:
                # Original logic for audit completion
                await crud.update_submission_status(db, submission_id, "Completed")
                logger.info(
                    f"[{AGENT_NAME}] Updated status to 'Completed' for submission {submission_id}."
                )

        except Exception as e:
            logger.error(
                f"[{AGENT_NAME}] Error saving results to DB for {submission_id}: {e}",
                exc_info=True,
            )
            return {
                "error": f"DB error: {e}",
...
def build_coordinator_graph():
    ...
    workflow.add_conditional_edges(
        ...
        {"estimate_cost": "estimate_cost", "end_with_error": "end_with_error"},
    )
    ...
    workflow.add_edge(
        "run_specialized_agents", "finalize_analysis"
    )  # This runs if cost_approval_met was true
