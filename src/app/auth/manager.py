...
from fastapi_users import BaseUserManager, UUIDIDMixin, models, schemas # Added models, schemas for type hints
...
if not SECRET_KEY:
    ...
    critical_error_msg = "SECRET_KEY environment variable not set! This is required for user management."
...
class UserManager(UUIDIDMixin, BaseUserManager[User, uuid.UUID]):
    ...
    async def on_after_forgot_password(
        ...
        """
        logger.info(f"User {user.id} ({user.email}) has requested a password reset. Token: {token[:6]}...") # [cite: 227]
    ...
    async def on_after_request_verify(
        ...
        """
        logger.info(f"User {user.id} ({user.email}) has requested a new verification token. Token: {token[:6]}...")
    ...
    #     created_user = await super().create(user_create, safe, request) # [cite: 228]
