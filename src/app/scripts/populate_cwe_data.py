...
async def main():
    ...
    try:
        ...
        logging.error(
            "Error: 'cwe_data_with_owasp.json' not found. Please run the processing script first."
        )
        return
    ...
    try:
        async with session_factory() as session:
            async with session.begin():
                ...
                if cwe_detail_objects:
                    logging.info(
                        f"Ingesting {len(cwe_detail_objects)} records into 'cwe_details' table..."
                    )
                    stmt_details = insert(CweDetail).values(cwe_detail_objects)
                    stmt_details = stmt_details.on_conflict_do_nothing(
                        index_elements=["id"]
                    )
                ...
                if cwe_owasp_mapping_objects:
                    logging.info(
                        f"Ingesting {len(cwe_owasp_mapping_objects)} records into 'cwe_owasp_mappings' table..."
                    )
                    stmt_mappings = insert(CweOwaspMapping).values(
                        cwe_owasp_mapping_objects
                    )
                    stmt_mappings = stmt_mappings.on_conflict_do_nothing(
        ...
        logging.error(
            f"A database error occurred during SQL ingestion: {e}", exc_info=True
        )
        return
    ...
    try:
        collection = rag_service._client.get_or_create_collection(
            name=CWE_RAG_COLLECTION_NAME
        )  # type: ignore
        ...
        batch_size = 200
        logging.info(
            f"Ingesting {len(ids)} documents into ChromaDB collection '{CWE_RAG_COLLECTION_NAME}'..."
        )
        for i in range(0, len(ids), batch_size):
            ...
            collection.add(
                ids=batch_ids, documents=batch_docs, metadatas=batch_metadatas
            )
