# src/app/agents/validation_agent.py
import logging
...
AGENT_DOMAIN_QUERY = "input validation, output encoding, Cross-Site Scripting (XSS), SQL Injection (SQLi), command injection, data sanitization, type checking, CWE-20, CWE-79, CWE-89"
...
async def assess_vulnerabilities_node(state: SpecializedAgentState) -> Dict[str, Any]:
    """
    Queries the RAG for relevant guidelines and uses the LLM to find validation and encoding vulnerabilities.
    """
    ...
    prompt = f"""
    You are a security expert specializing in {AGENT_NAME}. Analyze the following code snippet for vulnerabilities related to improper input validation and output encoding.
    Focus on issues like potential for Cross-Site Scripting (XSS) from unencoded output, SQL or other injections from unsanitized input, and failure to validate data against a strict schema.
    ...
    Identify validation and encoding vulnerabilities and respond with a structured list of findings. If no vulnerabilities are found, return an empty list.
    """
...
async def generate_fixes_node(state: SpecializedAgentState) -> Dict[str, Any]:
    ...
    for finding in findings:
        prompt = f"""
        A validation or encoding vulnerability has been identified in the following code snippet from file '{filename}'.
        ...
        Your task is to provide a secure code replacement for the vulnerable part. This should involve adding proper input sanitization, using parameterized queries for databases, or applying context-aware output encoding for web content.
...
def build_specialized_agent_graph():
    """Builds the LangGraph workflow for the Validation Agent."""
