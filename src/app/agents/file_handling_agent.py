# src/app/agents/file_handling_agent.py
import logging
...
AGENT_DOMAIN_QUERY = "secure file handling, file upload vulnerabilities, path traversal, directory traversal, unrestricted file upload, CWE-22, CWE-434"
...
async def assess_vulnerabilities_node(state: SpecializedAgentState) -> Dict[str, Any]:
    """
    Queries the RAG for relevant guidelines and uses the LLM to find file handling vulnerabilities.
    """
    ...
    prompt = f"""
    You are a security expert specializing in {AGENT_NAME}. Analyze the following code snippet for vulnerabilities related to file handling.
    Focus on issues like path traversal (directory traversal), unrestricted file uploads that could allow malicious file execution, and insecure handling of file paths and names from user input.
    ...
    Identify file handling vulnerabilities and respond with a structured list of findings. If no vulnerabilities are found, return an empty list.
    """
...
async def generate_fixes_node(state: SpecializedAgentState) -> Dict[str, Any]:
    ...
    for finding in findings:
        prompt = f"""
        A file handling vulnerability has been identified in the following code snippet from file '{filename}'.
        ...
        Your task is to provide a secure code replacement for the vulnerable part. This should involve validating user-supplied filenames, sanitizing paths to prevent traversal, and restricting file types on upload.
...
def build_specialized_agent_graph():
    """Builds the LangGraph workflow for the File Handling Agent."""
