...
AGENT_NAME = "SessionManagementAgent"
AGENT_DOMAIN_QUERY = "session management, insecure session handling, session fixation, session hijacking, cookie security, secure session tokens, session timeout, session termination"
...
async def assess_vulnerabilities_node(state: SpecializedAgentState) -> Dict[str, Any]:
    """
    Queries the RAG for relevant guidelines and uses the LLM to find session management vulnerabilities.
    """
    ...
    prompt = f"""
    You are a security expert specializing in {AGENT_NAME}. Analyze the following code snippet for vulnerabilities related to session management.
    Focus on issues like insecure cookie flags (e.g., missing HttpOnly, Secure, SameSite), predictable session tokens, lack of session timeout, and improper session invalidation upon logout.
    ...
    Identify session management vulnerabilities and respond with a structured list of findings. If no vulnerabilities are found, return an empty list.
    """
...
async def generate_fixes_node(state: SpecializedAgentState) -> Dict[str, Any]:
    ...
    for finding in findings:
        prompt = f"""
        A session management vulnerability has been identified in the following code snippet from file '{filename}'.
        ...
        Your task is to provide a secure code replacement for the vulnerable part. This may involve adding secure flags to cookies, regenerating session tokens upon login, or properly invalidating sessions.
...
def build_specialized_agent_graph():
    """Builds the LangGraph workflow for the Session Management Agent."""
