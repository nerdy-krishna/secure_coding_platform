...
class VulnerabilityFinding(BaseModel):
    ...
    cwe: str = Field(description="The most relevant CWE ID for this vulnerability, e.g., 'CWE-16'.")
...
async def assess_vulnerabilities_node(state: SpecializedAgentState) -> SpecializedAgentState:
    ...
    Your task is to analyze the following code or configuration snippet for vulnerabilities related to the OWASP ASVS V14 category.
    ...
    **Code Snippet ({state['language']}):**
    ...
    Analyze the code/configuration and identify any security misconfigurations or build/deployment vulnerabilities. For each finding, provide a detailed description, line number, severity, the most appropriate CWE ID, and a clear recommendation for fixing it.
