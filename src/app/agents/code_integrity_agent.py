# src/app/agents/code_integrity_agent.py
import logging
...
AGENT_NAME = "CodeIntegrityAgent"
AGENT_DOMAIN_QUERY = "code integrity, software supply chain security, dependency verification, insecure deserialization, code tampering, digital signatures, subresource integrity"
...
async def assess_vulnerabilities_node(state: SpecializedAgentState) -> Dict[str, Any]:
    """
    Queries the RAG for relevant guidelines and uses the LLM to find code integrity vulnerabilities.
    """
    ...
    prompt = f"""
    You are a security expert specializing in {AGENT_NAME}. Analyze the following code snippet for vulnerabilities related to code and data integrity.
    Focus on issues like insecure deserialization of untrusted data, use of unverified code or dependencies, and lack of integrity checks for external resources.
    ...
    Identify code integrity vulnerabilities and respond with a structured list of findings. If no vulnerabilities are found, return an empty list.
    """
...
async def generate_fixes_node(state: SpecializedAgentState) -> Dict[str, Any]:
    ...
    for finding in findings:
        ...
        Your task is to provide a secure code replacement for the vulnerable part. For insecure deserialization, this may involve using safer serialization formats or adding strict type validation.
...
def build_specialized_agent_graph():
    """Builds the LangGraph workflow for the Code Integrity Agent."""
