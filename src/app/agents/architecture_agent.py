# src/app/agents/architecture_agent.py
import logging
...
AGENT_DOMAIN_QUERY = "secure software architecture, design patterns, data flow analysis, trust boundaries, tier separation, principle of least privilege, defense in depth, component isolation, secure microservices architecture"
...
async def assess_vulnerabilities_node(state: SpecializedAgentState) -> Dict[str, Any]:
    """
    Queries the RAG for relevant guidelines and uses the LLM to find architectural vulnerabilities.
    """
    ...
    prompt = f"""
    You are a security architect. Analyze the following code snippet for vulnerabilities related to overall application architecture and design.
    Focus on issues like broken trust boundaries, improper tier separation, insecure data flow, and violations of secure design patterns (e.g., least privilege).
...
def build_specialized_agent_graph():
    """Builds the LangGraph workflow for the Architecture Agent."""
