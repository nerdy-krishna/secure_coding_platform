# src/app/agents/api_security_agent.py
import logging
...
AGENT_DOMAIN_QUERY = "API security, REST, GraphQL, rate limiting, insecure endpoints, exposed API keys, authentication tokens in APIs, authorization in APIs, excessive data exposure"
...
async def assess_vulnerabilities_node(state: SpecializedAgentState) -> Dict[str, Any]:
    """
    Queries the RAG for relevant guidelines and uses the LLM to find API security vulnerabilities.
    """
    ...
    prompt = f"""
    You are a security expert specializing in {AGENT_NAME}. Analyze the following code snippet, which may define API endpoints, clients, or related logic.
    Focus on vulnerabilities like insecure endpoint design, improper rate limiting, exposed credentials, and incorrect authorization checks.
    ...
    Identify API security vulnerabilities and respond with a structured list of findings. If no vulnerabilities are found, return an empty list.
    """
    llm_config_id = state.get("llm_config_id")
    if not llm_config_id:
        logger.error(f"[{AGENT_NAME}] LLM configuration ID not found in state.")
...
async def generate_fixes_node(state: SpecializedAgentState) -> Dict[str, Any]:
    ...
    llm_config_id = state.get("llm_config_id")
    if not llm_config_id:
        logger.warning(f"[{AGENT_NAME}] LLM configuration ID not found in state for fix generation. Skipping fixes.")
        return {"fixes": []}

    fixer_llm_client = await get_llm_client(llm_config_id=llm_config_id)
    if not fixer_llm_client:
        logger.warning(f"[{AGENT_NAME}] Failed to get LLM client for fix generation with config ID: {llm_config_id}. Skipping fixes.")
        return {"fixes": []}
...
def build_specialized_agent_graph():
    """Builds the LangGraph workflow for the API Security Agent."""
