...
class OpenAIProvider(LLMProvider):
    def __init__(self):
        ...
        if not self.api_key:
            raise ValueError(
                "OPENAI_API_KEY environment variable not set for OpenAIProvider."
            )
        try:
    ...
    async def generate(
        ...
        try:
            if generation_config_override:
                logger.warning(
                    "generation_config_override not directly applied to LangChain OpenAIProvider in this basic setup."
                )
            ...
            if isinstance(response.content, str):
                ...
                logger.error(
                    f"Unexpected response type from OpenAI: {type(response.content)}"
                )
                result.content = (
                    str(response.content) if response.content is not None else ""
                )
            ...
            if hasattr(response, "response_metadata") and isinstance(
                ...
                result.input_tokens = token_usage.get(
                    "prompt_tokens"
                ) or token_usage.get("input_tokens")
                result.output_tokens = token_usage.get(
                    "completion_tokens"
                ) or token_usage.get("output_tokens")
                result.total_tokens = token_usage.get("total_tokens")
                if result.total_tokens is not None:
                    logger.debug(
                        f"OpenAI Token Usage: Input={result.input_tokens}, Output={result.output_tokens}, Total={result.total_tokens}"
                    )
...
class GoogleGeminiProvider(LLMProvider):
    def __init__(self):
        ...
        if not self.api_key:
            raise ValueError(
                "GOOGLE_API_KEY environment variable not set for GoogleGeminiProvider."
            )
        ...
        try:
            ...
            logger.info(
                f"Initialized GoogleGeminiProvider with model: {self.model_name} using 'google-genai' SDK."
            )
        except Exception as e:
            logger.error(
                f"Failed to initialize Google Gemini client (model: {self.model_name}) using 'google-genai' SDK: {e}"
            )
            raise
        ...
        try:
            response = await self.client.generate_content_async(
                ...
                generation_config=genai.types.GenerationConfig(
                    **current_gen_config
                ),  # Pass as GenerationConfig object
            ...
            if (
                ...
                block_reason_message = (
                    ...
                    or "Blocked by safety settings"
                )
                logger.warning(
                    f"Google Gemini response blocked: {response.prompt_feedback.block_reason}. Message: {block_reason_message}"
                )
                ...
                ):  # MAX_TOKENS might not be an error per se
                    logger.warning(
                        f"Google Gemini generation finished with reason: {finish_reason_name}"
                    )
                    result.error = f"Generation stopped due to: {finish_reason_name}"
                    result.status = "failed"

            if (
                not result.content and result.status == "success"
            ):  # If no content but not explicitly blocked/failed
                logger.warning(
                    "Google Gemini response has no text content but was not explicitly blocked."
                )
            ...
            if hasattr(response, "usage_metadata") and response.usage_metadata:
                ...
                if result.total_tokens is not None:
                    logger.debug(
                        f"Google Gemini Token Usage: Input={result.input_tokens}, Output={result.output_tokens}, Total={result.total_tokens}"
                    )
                ...
                try:
                    ...
                    if result.content:
                        count_output_response = await self.client.count_tokens_async(
                            result.content
                        )
                    ...
                    logger.warning(
                        f"Could not manually count tokens for Gemini: {count_e}"
                    )

            if (
                result.status == "success" and not result.error
            ):  # If it wasn't set to failed/blocked
                result.status = "success"
            ...
            result.error = (
                f"Google API error: {e.message if hasattr(e, 'message') else str(e)}"
            )
            result.status = "failed"
        except Exception as e:
            end_time = time.perf_counter()
