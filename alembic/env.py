...
try:
    ...
    log.info(f"[ENV.PY] Target metadata set. Tables discovered: {list(target_metadata.tables.keys())}")
    if not list(target_metadata.tables.keys()):
        log.warning("[ENV.PY] WARNING: No tables found in Base.metadata. Check model definitions and imports.")

except ImportError as e:
    log.error(f"[ENV.PY] CRITICAL IMPORT ERROR: {e}. Autogenerate will likely fail or be empty.", exc_info=True)
...
def get_db_url_from_env():
    ...
    db_host = os.getenv("POSTGRES_HOST_ALEMBIC", os.getenv("POSTGRES_HOST", "localhost"))
...
def run_migrations_online() -> None:
    ...
    connectable = engine_from_config(
        config.get_section(config.config_ini_section, {}), # Pass the entire config section
        prefix="sqlalchemy.",
