...
import {
  getSeverityColor,
  getSeverityTagText,
} from "../../../shared/lib/severityMappings";
...
const getCvssScoreColor = (score: number | undefined): string => {
  ...
  if (score >= 9.0) return getSeverityColor("CRITICAL");
  if (score >= 7.0) return getSeverityColor("HIGH");
  if (score >= 4.0) return getSeverityColor("MEDIUM");
  if (score > 0.0) return getSeverityColor("LOW");
  return "default";
};
...
const FindingList: React.FC<FindingListProps> = ({
  ...
  if (findings.length === 0) {
    return (
      <Empty
        description="No findings in this file."
        style={{ marginTop: 48 }}
      />
    );
  }
  ...
  return (
    ...
    >
      {findings.map((finding) => {
        ...
        const isMergedFix =
          finding.fixes?.description &&
          finding.description === finding.fixes.description;
        ...
        return (
              ...
              >
                <Col>
                  <Space>
                    ...
                    <Text
                      copyable={{ text: String(finding.id) }}
                      type="secondary"
                    >
                      {" "}
                      ID: {finding.id}{" "}
                    ...
                    <Tag color={getSeverityColor(finding.severity)}>
                      {getSeverityTagText(finding.severity)}
                    </Tag>
                  </Space>
                </Col>
              </Row>
          ...
          >
            ...
            >
              ...
              <Descriptions.Item label="Confidence" span={1}>
                {" "}
                {finding.confidence}{" "}
              </Descriptions.Item>
              <Descriptions.Item label="CWE" span={1}>
                {" "}
                {finding.cwe}{" "}
              </Descriptions.Item>
              <Descriptions.Item label="CVSS Score" span={1}>
                <Tag color={getCvssScoreColor(finding.cvss_score)}>
                  {" "}
                  {finding.cvss_score?.toFixed(1) || "N/A"}{" "}
                </Tag>
              </Descriptions.Item>
              <Descriptions.Item label="CVSS Vector" span={2}>
                {" "}
                <Text code copyable>
                  {" "}
                  {finding.cvss_vector || "N/A"}{" "}
                </Text>
              </Descriptions.Item>
              <Descriptions.Item label="Corroborating Agents" span={2}>
                ...
                <Space wrap>
                  ...
                  finding.corroborating_agents.length > 0 ? (
                    finding.corroborating_agents.map((agent) => (
              ...
              <Descriptions.Item label="Line Number" span={2}>
                {" "}
                {finding.line_number}{" "}
              </Descriptions.Item>
              ...
              {isMergedFix && (
                <Descriptions.Item
                  label="Remediation Merge Explanation"
                  span={2}
                >
                  <Alert
                    type="info"
