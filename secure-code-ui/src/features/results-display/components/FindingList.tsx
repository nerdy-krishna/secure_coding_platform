...
import { getSeverityColor, getSeverityTagText } from "../../../shared/lib/severityMappings";
...
extra = {
  ...
  <Descriptions bordered column = { 2} size = "small" labelStyle = {{ backgroundColor: '#fafafa' }}>
    <Descriptions.Item label="Severity" span = { 1} > <Tag color={ getSeverityColor(finding.severity) }> { getSeverityTagText(finding.severity) } </Tag></Descriptions.Item>
                          ...
                          < Descriptions.Item label = "Description" span = { 2} > { finding.description } </Descriptions.Item>
{
  isMergedFix && (
    <Descriptions.Item label="Remediation Merge Explanation" span = { 2} >
      <Alert 
                        type="info"
  description = {< pre style = {{ whiteSpace: 'pre-wrap', fontFamily: 'inherit', margin: 0 }
}> { finding.fixes?.description } </pre>} 
                      />
  </Descriptions.Item>
                  )}
