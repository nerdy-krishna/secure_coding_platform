...
const buildTree = (
  paths: string[],
): { treeData: DataNode[]; folderKeys: React.Key[]; allKeys: React.Key[] } => {
  ...
  const convertToAntdFormat = (node: IntermediateTree): DataNode[] => {
    ...
    return sortedNodes.map((n: IntermediateNode) => ({
      title: (
        <span className={!n.isLeaf ? "folder-title" : "file-title"}>
          {n.title}
        </span>
      ),
      key: n.key,
      ...
      icon: n.isLeaf ? (
        ...
        <FolderTwoTone twoToneColor="#1677ff" />
      ),
...
const FileTree: React.FC<FileTreeProps> = ({ files, checkedKeys, onCheck }) => {
  const { treeData, folderKeys, allKeys } = useMemo(
    () => buildTree(files),
    [files],
  );
  ...
  return (
    <>
      ...
      >
        ...
        <Space wrap>
          <Button type="primary" size="small" onClick={() => onCheck(allKeys)}>
            Select All
          </Button>
          <Button type="primary" ghost size="small" onClick={() => onCheck([])}>
            Deselect All
          </Button>
          <Button
            type="primary"
