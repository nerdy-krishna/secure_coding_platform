...
const SetupPage: React.FC = () => {
    ...
    const handleCorsToggle = (e: React.ChangeEvent<HTMLInputElement>) => {
        setEnableCors(e.target.checked);
        setFormData({ ...formData, enable_cors: e.target.checked });
        // Clear allowed origins if disabled
        if (!e.target.checked) {
            setFormData(prev => ({ ...prev, allowed_origins: [], enable_cors: false }));
            setOriginInput("");
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload: SetupRequest = {
                ...
                allowed_origins: enableCors ? formData.allowed_origins : [],
            };
                    ...
                    {step === 3 && (
                        <>
                            <div style={{ marginBottom: '1rem' }}>
                                ...
                                <div style={{ marginBottom: '1rem', padding: '0.5rem', backgroundColor: '#f9fafb', borderRadius: '4px', border: '1px solid #e5e7eb' }}>
                                    <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', color: '#374151' }}>
                                        <input
                                            type="checkbox"
                                            checked={enableCors}
                                            onChange={handleCorsToggle}
                                            style={{ marginRight: '0.5rem', width: '16px', height: '16px' }}
                                        />
                                        Enable CORS Configuration (Optional)
                                    </label>
                                    <p style={{ marginTop: '0.25rem', fontSize: '0.75rem', color: '#6b7280', marginLeft: '1.5rem' }}>
                                        Check this if you are deploying to a production domain or accessing from a different origin.
                                    </p>
                                </div>
                                    ...
                                    <>
                                        <div style={{ marginBottom: '1rem', fontSize: '0.875rem', color: '#6b7280' }}>
                                            <p style={{ marginBottom: '0.5rem' }}>
                                                List the domains that are allowed to access the API (comma separated).
                                            </p>
                                            <ul style={{ listStyleType: 'disc', paddingLeft: '1.5rem' }}>
                                                <li><strong>Local Testing: </strong> Use <code>http://localhost:5173</code></li>
                                                <li><strong>Cloud / VPS: </strong> Use your domain, e.g., <code>https://your-domain.com</code></li>
                                            </ul>
                                        </div>
                                        <input
                                            type="text"
                                            placeholder="e.g., https://safe-code.example.com, http://localhost:5173"
                                            value={originInput}
