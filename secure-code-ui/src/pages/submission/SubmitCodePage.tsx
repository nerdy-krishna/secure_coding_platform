...
const SubmitCodePage: React.FC = () => {
  ...
  const renderContent = () => {
    if (submissionMode === "repo") {
      return (
        <Form.Item name= "repo_url" label = "Repository URL" style = {{ marginTop: 16 }
    } rules = { [{ required: true, message: "Please enter a valid repository URL." }, { type: "url", message: "The input is not a valid URL!" }]} >
...
label = {<> <ToolOutlined style={ { marginRight: 8 } } /> Utility Model</ >}
...
rules = { [{ required: true, message: "Please select the Utility LLM." }]}
  >
...
label = {<> <RobotOutlined style={ { marginRight: 8 } } /> Fast Model</ >}
...
rules = { [{ required: true, message: "Please select the Fast LLM." }]}
  >
...
label = {<> <SafetyCertificateOutlined style={ { marginRight: 8 } } /> Reasoning Model</ >}
...
rules = { [{ required: true, message: "Please select the Reasoning LLM." }]}
  >
...
{
  isPreviewComplete && (
    <Button type="link" onClick = {() => { setIsPreviewComplete(false); setPreviewFilePaths([]); }
} style = {{ marginTop: '10px' }} block >
