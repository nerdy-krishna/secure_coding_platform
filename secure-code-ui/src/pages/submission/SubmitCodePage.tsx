...
const SubmitCodePage: React.FC = () => {
  ...
  const getPathsFromUploadFiles = (files: UploadFile[]): string[] => {
    return files.map(
      (file) => (file.originFileObj as RcFile)?.webkitRelativePath || file.name,
    );
  };
  ...
  const handleFetchPreview = async () => {
    ...
    try {
      ...
      if (submissionMode === "repo") {
        ...
        fetchedPaths = await scanService.previewArchive(
          archiveFileList[0] as RcFile,
        );
      }
  ...
  const handleSubmit = async (values: SubmissionFormValues) => {
    ...
    try {
      ...
      formData.append(
        ...
        values.reasoning_llm_config_id,
      );
      ...
      if (submissionMode === "upload") {
        ...
        const filesToSubmit = fileList.filter((file) =>
          checkedFilePaths.has(
            (file.originFileObj as RcFile)?.webkitRelativePath || file.name,
          ),
        );
        ...
        if (archiveFileList[0].originFileObj) {
          formData.append(
            "archive_file",
            archiveFileList[0].originFileObj as RcFile,
          );
          formData.append("selected_files", treeCheckedKeys.join(","));
        }
      }
      ...
      navigate("/account/history", {
        state: { newProjectId: response.project_id },
      });
    } catch (error: unknown) {
    ...
    if (submissionMode === "repo") {
      return (
        <Form.Item
          ...
          style={{ marginTop: 16 }}
          rules={[
            { required: true, message: "Please enter a valid repository URL." },
        ...
        <Form.Item label="Upload Archive File" style={{ marginTop: 16 }}>
          <Dragger
            ...
            onChange={(info) => {
              ...
              if (latestFile[0] && !isArchiveFile(latestFile[0].name)) {
                message.error(
                  `File '${latestFile[0].name}' is not a recognized archive type.`,
                );
                setArchiveFileList([]);
              } else {
          ...
          >
            ...
            <p className="ant-upload-text">
              {" "}
              Click or drag a single archive file to this area{" "}
            </p>
          </Dragger>
        </Form.Item>
      );
    ...
    return (
      <Form.Item label="Upload Source Code Files" style={{ marginTop: 16 }}>
        ...
        >
          ...
          <p className="ant-upload-text">
            {" "}
            Click to select files, or drag and drop files and folders here{" "}
          </p>
        </Dragger>
      </Form.Item>
    );
  ...
  return (
    <Spin
      spinning={isSubmitting || isPreviewLoading}
      tip={
        isPreviewLoading
          ? "Fetching file list..."
          : "Submitting your code for analysis..."
      }
    >
      <Card>
        <Title level={2}> Submit Code for Analysis </Title>
        <Paragraph>
          Create or select a project, choose a scan type, and submit your code
        ...
        >
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Please enter or select a project name.",
              },
            ]}
          >
            <AutoComplete
          ...
          <Row gutter={24}>
            <Col xs={24} md={8}>
              <Form.Item
                name="utility_llm_config_id"
                label={
                  <>
                    {" "}
                    <ToolOutlined style={{ marginRight: 8 }} /> Utility Model
                  </>
                }
                tooltip="The 'Traffic Controller'. Checks your files to decide which security tests are needed. Keeps costs low by avoiding unnecessary scans. (e.g., GPT-3.5 Turbo)."
                rules={[
                  { required: true, message: "Please select the Utility LLM." },
                ]}
              >
                <Select
                  loading={isLoadingLLMs}
                  placeholder="Select a lightweight LLM"
            ...
            <Col xs={24} md={8}>
              <Form.Item
                name="fast_llm_config_id"
                label={
                  <>
                    {" "}
                    <RobotOutlined style={{ marginRight: 8 }} /> Fast Model
                  </>
                }
                tooltip="The 'Helper'. A balanced model for tasks like summarizing results and powering the chat assistant. Good speed and reasonable cost. (e.g., GPT-4o Mini)."
                rules={[
                  { required: true, message: "Please select the Fast LLM." },
                ]}
              >
                <Select
                  loading={isLoadingLLMs}
                  placeholder="Select a balanced LLM"
                  disabled={isLoadingLLMs || isLlmError}
                >
                  {llmConfigs?.map((config) => (
                    <Option key={config.id} value={config.id}>
            ...
            <Col xs={24} md={8}>
              <Form.Item
                ...
                tooltip="The 'Security Expert'. The smartest model available. Deeply analyzes code to find complex bugs and writes secure fixes. (e.g., GPT-4o, Claude 3.5 Sonnet)."
                rules={[
                  {
                    required: true,
                    message: "Please select the Reasoning LLM.",
                  },
                ]}
              >
                <Select
                  loading={isLoadingLLMs}
                  placeholder="Select a powerful LLM"
                  disabled={isLoadingLLMs || isLlmError}
                >
                  {llmConfigs?.map((config) => (
                    <Option key={config.id} value={config.id}>
          ...
          >
            <Radio.Group>
              <Tooltip title="The 'Health Check'. Scans your code to find vulnerabilities but makes no changes. Good for seeing where you stand.">
                <Radio.Button value="AUDIT"> Audit </Radio.Button>
              </Tooltip>
              <Tooltip title="The 'Advisor'. Finds vulnerabilities and shows you how to fix them, but lets *you* decide whether to apply the changes.">
                <Radio.Button value="SUGGEST"> Suggest Fixes </Radio.Button>
              </Tooltip>
              <Tooltip title="The 'Autofix'. Finds vulnerabilities and automatically applies fixes to your code. Great for speeding up security patches.">
          ...
          {(submissionMode === "repo" || submissionMode === "archive") &&
            !isPreviewComplete && (
              <Button
                block
          ...
          {(submissionMode === "upload" && fileList.length > 0) ||
          isPreviewComplete ? (
