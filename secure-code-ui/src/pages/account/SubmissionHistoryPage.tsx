...
const SubmissionHistoryPage: React.FC = () => {
  ...
  if (isError) {
    return (
      <Alert
        message="Error"
        description={`Could not fetch scan history: ${error.message}`}
        type="error"
        showIcon
      />
    );
  ...
  return (
    <Space direction="vertical" style={{ width: "100%" }} size="large">
      <Row justify="space-between" align="middle">
        <Col>
          <Title level={2}>
            <HistoryOutlined /> Submission History
          </Title>
          <Paragraph type="secondary">
            Review the status and results of all your individual scans.
          </Paragraph>
        </Col>
        <Col>
          {permission === "default" && (
      ...
      <Card>
        <Row gutter={[16, 16]} align="bottom">
          <Col xs={24} md={10}>
            <Paragraph style={{ marginBottom: 4, fontWeight: 500 }}>
              Search by Project, ID, or Status
            </Paragraph>
            <Input.Search
      ...
      {isLoading && (
        <div style={{ textAlign: "center", padding: "50px" }}>
          <Spin tip="Loading scans..." size="large" />
        </div>
      )}
          ...
          <Empty
            description={
              <>
                ...
                <Paragraph>
                  Get started by submitting code for your first project.
                </Paragraph>
