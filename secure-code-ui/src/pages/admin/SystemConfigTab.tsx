...
import { Table, Button, Modal, Form, Input, Switch, message, Popconfirm, Typography } from 'antd';
...
const SystemConfigTab: React.FC = () => {
    ...
    const createMutation = useMutation({
        mutationFn: (values: any) => apiClient.put(`/admin/system-config/${values.key}`, values),
        onSuccess: () => {
...
confirmLoading = { createMutation.isPending }
    ...
    <Form form={ form } layout = "vertical" >
        <Form.Item name="key" label = "Key" rules = { [{ required: true }]} >
            <Input disabled={ !!editingKey } placeholder = "e.g. GLOBAL_ALERT_MESSAGE" />
                </Form.Item>
                < Form.Item name = "value" label = "Value (JSON)" rules = { [{ required: true }]} >
                    <Input.TextArea rows={ 4 } placeholder = '{"message": "Maintenance Mode"}' />
                        </Form.Item>
                        < Form.Item name = "description" label = "Description" >
                            <Input placeholder="Short description of this setting" />
                                </Form.Item>
                                < Form.Item name = "is_secret" label = "Is Secret?" valuePropName = "checked" >
                                    <Switch />
                                    </Form.Item>
                                    </Form>
                                    </Modal>
