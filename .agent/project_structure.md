...
- **infrastructure/**:
    ...
    - **messaging/**: `outbox_sweeper.py` — background task that flushes unpublished `scan_outbox` rows to RabbitMQ. `prescan_approval_sweeper.py` — background task that auto-declines scans stuck at `PENDING_PRESCAN_APPROVAL` for >24 h (transitions to `BLOCKED_USER_DECLINE`, writes `PRESCAN_AUTO_DECLINED` scan event, deletes checkpointer thread). `retention_sweeper.py` — hourly task that deletes rows with `expires_at < NOW()` from `llm_interactions`, `chat_messages`, `rag_preprocessing_jobs` in FK-safe order (V14.2.7); kill switch via `RETENTION_SWEEPER_ENABLED=false`. All three are wired into `main.py` lifespan.
